class Player {
    constructor() {
        ...
        this.y = 0;
        this.speed = 10;
        this.size = 20; // Initial size of the player
        this.inCar = false;
    ...
    checkCollision() {
        ...
        for (const enemy of enemies) {
            ...
            if (
                ...
                // Player's car collided with an enemy
                this.handleCarEnemyCollision(enemy);
                console.log('Player\'s car collided with an enemy!');
            }
        }
    }


    handleCarEnemyCollision(enemy) {
        // Add your logic here for what happens when the car collides with an enemy
        // For example, reduce player's health, end the game, etc.
        // You can also remove the enemy from the array or perform other actions

        // For demonstration purposes, let's remove the enemy
...
class Car {
    constructor(isPlayerControlled) {
        if (currentPage !== 'bar_interior') {
            ...
            if (isPlayerControlled) {
                // Set up additional properties or controls for the player-controlled car
                this.element.classList.add('player-car');
                this.initializePlayerControls();
            } else {
                // Set up properties or behavior for non-player controlled cars
                this.initializeNonPlayerCar();
            }
        }
        ...
        initializePlayerControls() {
            // Example: Add placeholder controls for the player car (arrow keys)
            window.addEventListener('keydown', (event) => {
                switch (event.key) {
                    case 'ArrowUp':
                        // Implement actions for moving the player car up
                        break;
                    case 'ArrowDown':
                        // Implement actions for moving the player car down
                        break;
                    case 'ArrowLeft':
                        // Implement actions for turning the player car left
                        break;
                    case 'ArrowRight':
                        // Implement actions for turning the player car right
                        break;
                }
            });
        }
    
        initializeNonPlayerCar() {
            // Set the initial position of the non-player car
            ...
            // Set the initial angle (you can adjust this as needed)
            this.setAngle(0);
    
            // Start the movement for non-player cars
            this.startMoving();
        }
    ...
    startMoving() {
        // Set up an interval to continuously move the car forward
...
// Example: Create a player-controlled car
const playerCar = new Car(true);

// Example: Create non-player controlled cars
const nonPlayerCars = [];const bullets = [];
const enemies = [];
const keysPressed = {};
const reload_note=document.querySelector('.reloadText');

// Function to spawn a new non-player controlled car
function spawnNonPlayerCar() {
    const nonPlayerCar = new Car(false);
    nonPlayerCars.push(nonPlayerCar);
}

// Set up an interval to spawn a new non-player controlled car every 25 seconds
setInterval(() => {
    spawnNonPlayerCar();
}, 25000);

// Example: Update the game loop to continuously update both player and non-player cars
function updateGame() {
    // player.update();
    playerCar.update();


    for (const nonPlayerCar of nonPlayerCars) {
        nonPlayerCar.update();
    }
...
// Example: Create three non-player cars and add them to the array
for (let i = 0; i < 1; i++) {
    const nonPlayerCar = new Car(false);
    nonPlayerCars.push(nonPlayerCar);
}
...
gameLoop();
        ...
        //     break;
                        case 'Space':
                            ...
                            const bullet = new Bullet(player.x, player.y, 15, { x: 1, y: 0 });
                            bullets.push(bullet);
                            break;
                            case 'c':
