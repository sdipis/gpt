...
for (let i = 0; i < 
    ...
    for (let i = 0; i < 
        50 //patrons will spawn in the outside portions
        ; i++) {
        enemies.push(new Enemy());
    }
}
...
/* This is the main brain of the   /
   ...
   function gameLoop(player, sundayDriver, enemies, bullets, items) {
    // Update player
    player.update();
    sundayDriver.update();
    ...
    checkCollisionWithEnemies(player, enemies, bullets);
    checkCarCollisionWithEnemies(player, sundayDriver, enemies);
    // checkPlayerCollisionsWithItems(player, items);
...
for (const item of items) {
    ...
    requestAnimationFrame(() => gameLoop(player, sundayDriver, enemies, bullets, items));
}

function updateHUD(hud) {
    // Check if hud is defined
    if (hud && typeof hud.update === 'function') {
        hud.update();
    }}

// Call the gameLoop function to start the game loop
gameLoop(player, sundayDriver, enemies, bullets, items);
